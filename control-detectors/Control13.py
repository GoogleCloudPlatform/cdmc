...
class Control13:
    ...
    def generateReport(self):
        ...
        print("Verifying Control 13" )
        results = searchCatalogAssets(self.org_id,self.project_id,str(config["DC_FILTERS"]["Control13"]))
        for result in results:
            message = {
                "reportMetadata":self.report_metadata,
                "CdmcControlNumber":13,
                "Findings":str(config["FINDINGS"]["Control13"]),
                "DataAsset":str(result.linked_resource),
                "RecommendedAdjustment":str(config["RECOMMENDATIONS"]["Control13"]),
